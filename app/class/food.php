...
class Food extends Db{
    ...
    public function removeFromBreakfast($idUser, $idFood, $date){
        $sql = "DELETE FRPM desayuno WHERE idUser = ? AND idAlimento = ? AND  fecha = ?;";
        $query = $this->connect()->prepare($sql);  
        if ($query->execute([$idUser, $idFood, $date])) {
                return true;
        }else{
    ...
    public function removeFromLaunch($idUser, $idFood, $date){
        $sql = "DELETE FRPM comida WHERE idUser = ? AND idAlimento = ? AND  fecha = ?;";
        $query = $this->connect()->prepare($sql);  
        if ($query->execute([$idUser, $idFood, $date])) {
                return true;
        }else{
    ...
    public function removeFromDinner($idUser, $idFood, $date){
        $sql = "DELETE FRPM cena WHERE idUser = ? AND idAlimento = ? AND  fecha = ?;";
        $query = $this->connect()->prepare($sql);  
        if ($query->execute([$idUser, $idFood, $date])) {
                return true;
        }else{
